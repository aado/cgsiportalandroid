 ...
 *
 */
class ForceLeaveMaternity extends CI_Controller
{
	/**
	 * User data
	 * 
	...
	public function index()
	{
		...
		// print_r($this->Cgsi_model->getDataForfLMat());
		$data = [
			'page' => [
				'title' => 'Force Leave / Maternity'
			],
			'username' => $this->session->username,
			'user' => $this->Cgsi_model->getUserInMasterlist(),
			'torta' => $this->Cgsi_model->getDataForfLMat(),
			'status' => $status
		];
		$this->load->view('template/header',$data);
		$this->load->view('forceleave', $data);
