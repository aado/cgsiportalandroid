...
$( document ).ready( function () {
				...
				api
					...
					.each(function (group, i) {
						if (last !== group) {
							$(rows)
								.eq(i)
								.before('<tr class="group"><td colspan="6"><b>' + group + '</b></td></tr>');
	...
	var groupColumn2 = 2;
		var table2 = $('#MISitemtransmittedNew').DataTable({
			columnDefs: [{ visible: false, targets: groupColumn2 }],
				...
				api
					...
					.each(function (group, i) {
						if (last !== group) {
							$(rows)
								.eq(i)
								.before('<tr class="group" style="background: darkslateblue;"><td colspan="9"><b>' + group + '</b></td></tr>');
	...
	$('#MISitemtransmittedhistory').DataTable({
		responsive: true
	});
...
function getDataPullOut_Existing(qty,trnsno,trnsto, dtrns, pulloutno, particulars, brand, toolm, serialno, polid) {
	console.log(trnsno+'-'+trnsto+ '-'+ dtrns);
	var pulloutData = trnsno+'~'+trnsto+ '~'+ dtrns;
	var pulloutDetails = pulloutno+'~'+particulars+'~'+brand+'~'+toolm+'~'+serialno+'~'+polid;
	$("#pulloutData").val(pulloutData);
	$("#pulloutDetails").val(pulloutDetails);
	$("#pulloutqty1").html("");
	console.log(qty);
	var optionSelect="";
	for(i=1; i<=qty;i++){
	...
	$("#pulloutqty1").append(optionSelect);
}

function getDataPullOut(qty,trnsno,trnsto, dtrns, pulloutno, particulars, brand, toolm, serialno, polid) {
	console.log(trnsno+'-'+trnsto+ '-'+ dtrns);
	var pulloutData = trnsno+'~'+trnsto+ '~'+ dtrns;
	var pulloutDetails = pulloutno+'~'+particulars+'~'+brand+'~'+toolm+'~'+serialno+'~'+polid;
	$("#pulloutData").val(pulloutData);
	...
	$("#pulloutqty2").append(optionSelect);
}
